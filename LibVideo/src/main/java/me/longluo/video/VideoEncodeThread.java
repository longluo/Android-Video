...
import static me.longluo.video.VideoProcessor.DEFAULT_FRAME_RATE;
...
public class VideoEncodeThread extends Thread implements IVideoEncodeThread {
    ...
    public VideoEncodeThread(MediaExtractor extractor, MediaMuxer muxer,
        ...
        Timber.i("VideoEncodeThread bitrate = %s, resultWidth = %s, resultHeight = %s, " +
                "iFrameInterval = %s, frameRate = %s, videoIndex = %s", bitrate, resultWidth,
                resultHeight, iFrameInterval, frameRate, videoIndex);
    ...
    private void doEncode() throws IOException {
        ...
        if (supportProfileHigh) {
            Timber.i("supportProfileHigh,enable ProfileHigh");
        }

        int maxBitrate = VideoUtil.getMaxSupportBitrate(mEncoder, mimeType);
        if (maxBitrate > 0 && mBitrate > maxBitrate) {
            Timber.e(mBitrate + " bitrate too large,set to:" + maxBitrate);
...
//        mInputSurface.makeCurrent();
        ...
        while (true) {
            ...
            Timber.i("encode outputBufferIndex = " + outputBufferIndex);
            if (signalEncodeEnd && outputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (encodeTryAgainCount > 20) {
                    ...
                    Timber.e("INFO_TRY_AGAIN_LATER 20 times,force End!");
                    break;
                }
            } else {
            ...
            if (outputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                Timber.i("encode newFormat = " + newFormat);
                ...
                Timber.i("writeSampleData,size:" + info.size + " time:" + info.presentationTimeUs / 1000);
                mMuxer.writeSampleData(videoTrackIndex, outputBuffer, info);
                notifyProgress(info);
                mEncoder.releaseOutputBuffer(outputBufferIndex, false);
                if (info.flags == MediaCodec.BUFFER_FLAG_END_OF_STREAM) {
                    Timber.i("encoderDone");
                    break;
                }
            }
        }

        Timber.i("Video Encode Done!");
    }
