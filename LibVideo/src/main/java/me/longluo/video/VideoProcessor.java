...
import static me.longluo.video.util.AudioUtils.getAudioBitrate;
...
import me.longluo.video.util.PcmToWavUtils;
import me.longluo.video.util.VideoMultiStepProgress;
import me.longluo.video.util.VideoProgressAve;
...
public class VideoProcessor {
    ...
    public static void processVideo(@NotNull Context context, @NotNull Processor processor) throws Exception {
        ...
        if (audioIndex >= 0) {
            ...
            AudioUtils.checkCsd(audioEncodeFormat,
        ...
        try {
            ...
            Timber.w(String.format("编解码:%dms,音频:%dms", e1 - s, e2 - s));
    ...
    public static void adjustVideoVolume(Context context, final MediaSource mediaSource, final String output, int videoVolume, float faceInSec, float fadeOutSec) throws IOException {
        if (videoVolume == 100 && faceInSec == 0f && fadeOutSec == 0f) {
            AudioUtils.copyFile(mediaSource, output);
            return;
        }
        ...
        AudioUtils.decodeToPCM(mediaSource, videoPcmFile.getAbsolutePath(), null, null);
        AudioUtils.adjustPcmVolume(videoPcmFile.getAbsolutePath(), videoPcmAdjustedFile.getAbsolutePath(), videoVolume);
        ...
        if (faceInSec > 0 || fadeOutSec > 0) {
            AudioFadeUtils.audioFade(videoPcmAdjustedFile.getAbsolutePath(), sampleRate, channelCount, faceInSec, fadeOutSec);
        }
        new PcmToWavUtils(sampleRate, channelConfig, channelCount, AudioFormat.ENCODING_PCM_16BIT).pcmToWav(videoPcmAdjustedFile.getAbsolutePath(), videoWavFile.getAbsolutePath());
        ...
        try {
            while (!encodeDone) {
                ...
                if (!encodeInputDone && inputBufferIndex >= 0) {
                    ...
                    if (sampleTime < 0) {
                        ...
                        Timber.i("audio queuePcmBuffer " + sampleTime / 1000 + " size:" + size);
                ...
                while (true) {
                    ...
                    if (outputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                        ...
                        Timber.i("audio decode newFormat = " + newFormat);
                    } else if (outputBufferIndex < 0) {
                        //ignore
                        Timber.e( "unexpected result from audio decoder.dequeueOutputBuffer: " + outputBufferIndex);
                    } else {
                        ...
                        Timber.i("audio writeSampleData " + info.presentationTimeUs + " size:" + info.size + " flags:" + info.flags);
                        if (!detectTimeError && lastAudioFrameTimeUs != -1 && info.presentationTimeUs < lastAudioFrameTimeUs + AAC_FRAME_TIME_US) {
                            ...
                            Timber.e("audio 时间戳错误，lastAudioFrameTimeUs:" + lastAudioFrameTimeUs + " " +
                        ...
                        if (detectTimeError) {
                            ...
                            Timber.e("audio 时间戳错误，使用修正的时间戳:" + info.presentationTimeUs);
            ...
            while (true) {
                ...
                if (!detectTimeError && lastVideoFrameTimeUs != -1 && info.presentationTimeUs < lastVideoFrameTimeUs + VIDEO_FRAME_TIME_US) {
                    //某些视频帧时间会出错
                    Timber.e("video 时间戳错误，lastVideoFrameTimeUs:" + lastVideoFrameTimeUs + " " +
                ...
                if (detectTimeError) {
                    ...
                    Timber.e("video 时间戳错误，使用修正的时间戳:" + info.presentationTimeUs);
                    detectTimeError = false;
                }
                if (info.flags != MediaCodec.BUFFER_FLAG_CODEC_CONFIG) {
                    lastVideoFrameTimeUs = info.presentationTimeUs;
                }
                Timber.w("video writeSampleData:" + info.presentationTimeUs + " type:" + info.flags + " size:" + info.size);
    ...
    public static void mixAudioTrack(Context context, final MediaSource videoInput, final MediaSource audioInput, final String output,
        ...
        if (oriAudioIndex >= 0) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        AudioUtils.decodeToPCM(videoInput, videoPcmFile.getAbsolutePath(), startTimeUs, endTimeUs);
                    } catch (IOException e) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        AudioUtils.decodeToPCM(audioInput, finalAacPcmFile.getAbsolutePath(), 0, aacDurationMs > videoDurationMs ? videoDurationMs * 1000 : aacDurationMs * 1000);
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    } finally {
                        latch.countDown();
            ...
            Pair<Integer, Integer> resultPair = AudioUtils.checkAndAdjustAudioFormat(videoPcmFile.getAbsolutePath(),
            ...
            //检查音频长度是否需要重复填充
            if (AUDIO_MIX_REPEAT) {
                aacPcmFile = AudioUtils.checkAndFillPcm(aacPcmFile, aacDurationMs, videoDurationMs);
            }

            //混合并调整音量
            adjustedPcm = new File(cacheDir, "adjusted_" + System.currentTimeMillis() + ".pcm");
            AudioUtils.mixPcm(videoPcmFile.getAbsolutePath(), aacPcmFile.getAbsolutePath(), adjustedPcm.getAbsolutePath()
            ...
            if (fadeInSec != 0 || fadeOutSec != 0) {
                AudioFadeUtils.audioFade(adjustedPcm.getAbsolutePath(), sampleRate, channelCount, fadeInSec, fadeOutSec);
            }
            //PCM转WAV
            new PcmToWavUtils(sampleRate, channelConfig, channelCount, AudioFormat.ENCODING_PCM_16BIT).pcmToWav(adjustedPcm.getAbsolutePath(), wavFile.getAbsolutePath());
            long s5 = System.currentTimeMillis();
            Timber.e(String.format("decode:%dms,resample:%dms,mix:%dms,fade:%dms", s2 - s1, s3 - s2, s4 - s3, s5 - s4));
            ...
            AudioUtils.checkCsd(oriAudioFormat,
                    MediaCodecInfo.CodecProfileLevel.AACObjectLC,
                    sampleRate,
                    channelCount
            );
            ...
            AudioUtils.decodeToPCM(audioInput, aacPcmFile.getAbsolutePath(), 0,
            ...
            AudioUtils.checkCsd(audioTrackFormat,
                    MediaCodecInfo.CodecProfileLevel.AACObjectLC,
                    sampleRate,
                    channelCount);
            ...
            if (channelCount > 2) {
                ...
                AudioUtils.stereoToMonoSimple(aacPcmFile.getAbsolutePath(), tempFile.getAbsolutePath(), channelCount);
                channelCount = 1;
                aacPcmFile.delete();
                aacPcmFile = tempFile;
            }
            ...
            if (aacVolume != 50) {
                adjustedPcm = new File(cacheDir, "adjusted_" + System.currentTimeMillis() + ".pcm");
                AudioUtils.adjustPcmVolume(aacPcmFile.getAbsolutePath(), adjustedPcm.getAbsolutePath(), aacVolume);
            } else {
